...
struct XlsxHyperlinkData {
    ...
    XlsxHyperlinkData(LinkType linkType = External, const QString& target = QString(), const QString& location = QString(), const QString& display = QString(),
...
struct XlsxSheetFormatProps {
    XlsxSheetFormatProps(int baseColWidth = 8, bool customHeight = false,
        ...
        double defaultRowHeight = 15, quint8 outlineLevelCol = 0, quint8 outlineLevelRow = 0, bool thickBottom = false, bool thickTop = false,
...
struct XlsxColumnInfo {
    XlsxColumnInfo(int firstColumn, // = 0,
        int lastColumn, // = 1,
        bool isSetWidth, double width = 0, const Format& format = Format(), bool hidden = false)
        : width(width)
        , format(format)
...
public:
    ...
    std::shared_ptr<Cell> cellAt(int row, int column) const { return cells.value(row).value(column); }
