...
{
    if (hasProperty(FormatPrivate::P_Alignment_Indent) && (align != AlignHGeneral && align != AlignLeft && align != AlignRight && align != AlignHDistributed)) {
        clearProperty(FormatPrivate::P_Alignment_Indent);
    }

    if (hasProperty(FormatPrivate::P_Alignment_ShinkToFit) && (align == AlignHFill || align == AlignHJustify || align == AlignHDistributed)) {
...
/*!
 * \internal
 *  Called by YXlsx::Styles or some unittests.
 */
...
{
    dbg.nospace() << "YXlsx::Format(" << f.d->properties << ")";
    return dbg.space();
}
#endif
